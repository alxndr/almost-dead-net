...
function ListItem({date, performancesOnDate}) {
  ...
  return <li key={performanceData.id} className={classnames({highlight: window.previousPath?.endsWith(url)})}>
    <Link to={url}>
      {date}
      {' '}
    ...
    ? <>
      ...
      <ul>
        {teases.map(teaseData => {
          ...
          const url = `/show/${showData.id}`
          return <li key={teaseData.id} className={classnames({highlight: window.previousPath?.endsWith(url)})}>
